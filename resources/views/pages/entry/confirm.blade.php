...
@section('content')
    ...
    /* @var $page */
    if(isset($page->fEntConfig->backendSettings['form']['entry']) && count($page->fEntConfig->backendSettings['form']['entry'])>0) {

        foreach($page->fEntConfig->backendSettings['form']['entry'] As $sectionName => $items) {
        ...
        //カスタム項目の格納処理
        if(isset($page->fEntConfig->backendSettings['form']['entry']['custom']) && count($page->fEntConfig->backendSettings['form']['entry']['custom'])>0) {
            foreach($page->fEntConfig->backendSettings['form']['entry']['custom'] As $sectionName => $items) {
                foreach($items As $groupName => $list) {
                    foreach($list As $fieldName => $rules) {
                ...
                <div class="inner">
                    ...
                    <section class="mod_jobDetailJob">
                        <section class="mod_apply_confirm">
                            <form id="entryConfirmForm" name="fm" method="post" action="{{$page->action ?? '#'}}">
                                @csrf
                        ...
                        @endphp
                                ...
                                <aside class="applyBtns">
                                    <a class="back" href="javascript:void(0)" id="applyback">戻る</a>
                                    <button class="submitApply" type="submit" value="" {{$isTrueDate ? '' : 'disabled'}}>登録する</button>
                                </aside>
                            </form>
    ...
    <script>
        function pageBack(e) {
            e.preventDefault();
            let form = document.getElementById('entryConfirmForm');
