@php
    ...
    if(isset($fEntConfig->backendSettings['form']['entry']) && count($fEntConfig->backendSettings['form']['entry'])>0) {

        foreach($fEntConfig->backendSettings['form']['entry'] As $sectionName => $items) {
        ...
        //カスタム項目の格納処理
        if(isset($fEntConfig->backendSettings['form']['entry']['custom']) && count($fEntConfig->backendSettings['form']['entry']['custom'])>0) {
            foreach($fEntConfig->backendSettings['form']['entry']['custom'] As $sectionName => $items) {
                foreach($items As $groupName => $list) {
                    foreach($list As $fieldName => $rules) {
                        $formRegistSettings[$sectionName][$groupName][$fieldName] = $rules;
                    }
