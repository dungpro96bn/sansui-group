...
class LatestJobController extends Controller
{
    ...
    /**
     * LatestJobController constructor.
    ...
    public function store()
    {
        ...
        $newJobInfo = array();
        if(isset($page->fEntConfig->backendSettings['news'])) {
            $newJobInfo = $page->fEntConfig->backendSettings['news'];
        }

        $term = $newJobInfo['term']??7;
        $k = time() - ($term * 24 * 60 * 60);

        $fEntJobSearchCriteria->termFrom = date('Y-m-d', $k);
        $fEntJobSearchCriteria->termTo = date('Y-m-d');
        $fEntJobSearchCriteria->pageLimit = $newJobInfo['limit']??3;
        ...
        if(isset($this->page->fEntConfig->frontendSettings['arrayCorpCd']) &&(count($this->page->fEntConfig->frontendSettings['arrayCorpCd'])>0) ) {
            ...
            $list = view('components.latestJob.item.swiperNoImage', compact('arrayFEntJob'))->render();
        ...
        $fEntJobSearchCriteria->sort = $newJobInfo['sort']??null;
        ...
        $list = view('components.latestJob.item.swiperNoImage', compact('arrayFEntJob', 'favoriteList'))->render();
